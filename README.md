...
- The Coaching Report's printed PDF preserves the dark Wenger theme — Navy background, gradient title, brand-colored category bars and pills — using `-webkit-print-color-adjust: exact` so browsers render backgrounds faithfully on paper. Each rep section uses `break-inside: avoid` and starts on a new page when previous content fills.
