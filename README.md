...
- The Coaching Report's printed PDF uses a separate clean Wenger-branded paper layout (light background, Bebas Neue navy headings, leaf / bou / burnt-orange accents, tidy tables, priority pills for action items). The on-screen rich dark report is unchanged. Switching to a paper-friendly printable view fixed a Chrome multi-page quirk that was clipping later reps from the PDF.
